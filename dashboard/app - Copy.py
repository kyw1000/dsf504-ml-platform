...
_nav_options = list(PAGES.keys())
if "nav_page" not in st.session_state:
    st.session_state["nav_page"] = _nav_options[0]

_cur_page = st.session_state.get("nav_page", _nav_options[0])
if _cur_page not in _nav_options:
    _cur_page = _nav_options[0]

nav_choice = st.radio(
    "Page",
    _nav_options,
    index=_nav_options.index(_cur_page),
    horizontal=True,
    key="nav_page",
    label_visibility="collapsed",
)
